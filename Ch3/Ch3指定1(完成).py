...
ceiling = box(length=0.4, height=0.005, width=0.4, opacity = 0.2,pos=vec(0,L,0))#畫天花板
ball = sphere(radius = size,  color=color.yellow, make_trail = False, retain = 1000, interval=1)#畫球
...
ball.pos = vector(0, 0, 0)     #球在t=0時的位置
ball.v = vector(0, 0, 0)        #球初速
spring.pos = vector(0, L, 0)    #彈簧頭端的位置
...
while True:
    ...
    f1.plot(pos=(t,0.5 * m * mag(ball.v)**2))
    f2.plot(pos=(t,0.5 * k * (mag(spring.axis) - L)**2))
    f3.plot(pos=(t,m * g * ball.pos.y))
    f4.plot(pos=(t,0.5 * m * mag(ball.v)**2+0.5 * k * (mag(spring.axis) - L)**2+m * g * ball.pos.y))
